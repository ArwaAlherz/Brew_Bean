...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['update_product'])) {
    ...
    if (isset($_FILES['image']) && $_FILES['image']['error'] === 0) {
        ...
        $img_name_esc = mysqli_real_escape_string($conn, $img_name);
        $image_update = ", image='$img_name_esc'";
    }
        ...
        <div class="right-side">
          ...
          <?php foreach ($products as $product) { ?>
            ...
            <div class="panel edit-panel" id="edit-<?php echo $product['product_id']; ?>">
              ...
              <form class="form" method="post" action="view_products.php" enctype="multipart/form-data" onsubmit="return validateProduct(this)">
                <!-- معاينة الصورة الحالية + رفع جديدة -->
                <div style="display:flex;align-items:center;gap:14px;background:#FAF3EA;border:1px solid #E2D2BE;border-radius:14px;padding:14px;">
                  <div class="p-img" style="width:100px;height:100px;flex-shrink:0;">
                    <img id="imgPreview-<?php echo $product['product_id']; ?>"
                  ...
                  <div style="flex:1">
                    <label style="font-size:12px;font-weight:700;color:#6B5A50;">الصورة الحالية</label>
                    <p style="font-size:12px;color:#6B5A50;margin:2px 0 10px;word-break:break-all;"><?php echo htmlspecialchars($product['image']); ?></p>
                    <label style="font-size:12px;font-weight:700;color:#6B5A50;">رفع صورة جديدة (اختياري)</label>
                    <input type="file" name="image" accept="image/*"
                           style="font-size:12px;margin-top:4px;width:100%;"
                           onchange="previewImg(this,'imgPreview-<?php echo $product['product_id']; ?>')">
                  </div>
                </div>
